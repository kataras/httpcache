...
//
// One function, accepts the request context and returns true if should be denied/ignore, otherwise false.
...
type Denier func(*fasthttp.RequestCtx) bool
...
var DefaultDeniers = []Denier{
	...
	func(reqCtx *fasthttp.RequestCtx) bool {
		...
		return get("must-revalidate") != "" ||
			get("s-maxage") == "0" ||
			get("max-age") == "0" ||
			get("X-No-Cache") != "" // this is a custom header, for any case ( I use that in my projects so I recommend and introduce this here too*)
	},
}
